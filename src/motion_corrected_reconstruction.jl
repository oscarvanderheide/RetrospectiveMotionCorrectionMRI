...
mutable struct OptionsMotionCorrection
    options_imrecon::OptionsImageReconstruction
    options_parest::OptionsParameterEstimation
    niter::Integer
...
motion_correction_options(; image_reconstruction_options::OptionsImageReconstruction, parameter_estimation_options::OptionsParameterEstimation, niter::Integer, niter_estimate_Lipschitz::Union{Nothing,Integer}=nothing, verbose::Bool=false, fun_history::Bool=false) = OptionsMotionCorrection(image_reconstruction_options, parameter_estimation_options, niter, niter_estimate_Lipschitz, verbose, fun_history ? Vector{NTuple{2,Any}}(undef,niter) : nothing)

AbstractProximableFunctions.fun_history(options::OptionsMotionCorrection) = options.fun_history


## Motion-corrected reconstruction algorithms

function motion_corrected_reconstruction(F::StructuredNFFTtype2LinOp{T}, d::AbstractArray{CT,2}, u::AbstractArray{CT,3}, θ::AbstractArray{T,2}; options::Union{Nothing,OptionsMotionCorrection}=nothing) where {T<:Real,CT<:RealOrComplex{T}}
