...
for experiment_subname = ["vol1_priorT1"], motion_type = [2, 3]
    ...
    @save string(results_folder, "results_", experiment_subname, "_motion", motion_type, "_baseline.jld") u θ psnr_recon psnr_conv ssim_recon ssim_conv u_reg corrupted_reg ground_truth_reg
    plot_volume_slices(abs.(u_reg); spatial_geometry=X, vmin=vmin, vmax=vmax, savefile=string(figures_subfolder, "corrected_motion", string(motion_type), "_baseline.png"), orientation=orientation)
    plot_volume_slices(abs.(ground_truth_reg); spatial_geometry=X, vmin=vmin, vmax=vmax, savefile=string(figures_subfolder, "ground_truth_reg_baseline.png"), orientation=orientation)
    plot_volume_slices(abs.(corrupted_reg); spatial_geometry=X, vmin=vmin, vmax=vmax, savefile=string(figures_subfolder, "corrupted_reg_motion", string(motion_type), "_baseline.png"), orientation=orientation)
    ...
    plot_parameters(1:size(θ,1), θ, nothing; xlabel="t = phase encoding", vmin=vec(θ_middle-1.1f0*Δθ), vmax=vec(θ_middle+1.1f0*Δθ), fmt1="b", linewidth1=2, savefile=string(figures_subfolder, "parameters_motion", string(motion_type), "_baseline.png"))
    close("all")
