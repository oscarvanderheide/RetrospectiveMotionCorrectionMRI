...
for (i, scale) in enumerate(n_scales:-1:0), (j, ε_rel) in enumerate(ε_schedule)
    ...
    nt_h = size(F_h.kcoord)[1]
    data_h = resample(data, K_h; norm_constant=F.norm_constant/F_h.norm_constant)
    ...
    # Optimization options:
        ...
        opt_inner = FISTA_optimizer(4f0*sum(1 ./h.^2); Nesterov=true, niter=5)
    ...
    idx_θ_h = K_h.idx_phase_encoding
