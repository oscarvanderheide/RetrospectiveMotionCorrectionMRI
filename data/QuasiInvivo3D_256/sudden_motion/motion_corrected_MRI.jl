...
for i = 1:scale_max####
    ...
    u_conventional_h = downscale(u_conventional, X; fact=scale)
    ...
    η = 1f-1*structural_mean(prior_h)
    ...
    (scale != 0) ? (ε = 0.05f0*g(u_conventional_h)) : (ε = 0.3f0*g(u_conventional_h))
    ...
    nt_h, _ = size(F_h.K)
    ti = Float32.(1:nt_h)
    # ti = Float32.(range(1, nt_h; length=2^(7+i-1)+1))
    t  = Float32.(1:nt_h)
